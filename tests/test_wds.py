...
from src.open_clip.src.open_clip_train.data import get_wds_dataset
from src.open_clip.src.open_clip_train.params import parse_args
from src.open_clip.src.open_clip_train.main import random_seed
...
def test_two_sources_with_upsampling():
    ...
    for key, count in counts.items():
        if key.startswith("000"):
            assert count == pytest.approx(TRAIN_NUM_SAMPLES / 20, RTOL), (
                f"{key}, {count}"
            )
        else:
            assert count == pytest.approx(TRAIN_NUM_SAMPLES / 10, RTOL), (
