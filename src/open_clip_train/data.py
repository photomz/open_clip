...
def get_hf_dataset(
    args, preprocess_fn, is_train, epoch=0, tokenizer1=None, tokenizer2=None, **kw
):
    """
    ...
    def collate_fn(batch):
        X0 = [item["input"] for item in batch]
        Y0 = [item["output"] for item in batch]
        X1 = tokenizer1(X0)
        Y1 = tokenizer2(Y0)
        # debug(X1.shape, Y1.shape)
        X2 = torch.tensor(X1)
        Y2 = torch.tensor(Y1)
        # debug(X0, X1.shape, X2.shape)
        return (X2, Y2)

    # # Create DataLoader
    # collate_fn = lambda batch: (
    #     torch.stack(tokenizer1([item["input"]]) for item in batch),
    #     torch.stack([tokenizer2([item["output"]]) for item in batch]),
    # )
...
def get_data(args, preprocess_fns, epoch=0, tokenizer=None, **kwargs):
    preprocess_train, preprocess_val = preprocess_fns
    data = {}
    ...
    if args.val_data:
        data["val"] = get_dataset_fn(args.val_data, args.dataset_type)(
            args, preprocess_val, is_train=False, tokenizer=tokenizer, **kwargs
        )
