...
class TextTextCLIP(nn.Module):
    ...
    def __init__(
        self,
        embed_dim: int,
        text_cfg1: CLIPTextCfg,
        text_cfg2: CLIPTextCfg,
        quick_gelu: bool = False,
        init_logit_scale: float = np.log(1 / 0.07),
        init_logit_bias: Optional[float] = None,
        nonscalar_logit_scale: bool = False,
        ...
        self.text1 = _build_text_tower(embed_dim, text_cfg1, quick_gelu, cast_dtype)
        self.text2 = _build_text_tower(embed_dim, text_cfg2, quick_gelu, cast_dtype)
        # define text1 := image, text2 := text
        # for external-facing OpenCLIP utils.
        self.text = self.text1
        self.visual = self.text2
    ...
    def lock_text_tower(self, unlocked_layers: int = 0, freeze_layer_norm: bool = True):
        self.text2.lock(unlocked_layers, freeze_layer_norm)

    @torch.jit.ignore
    def set_grad_checkpointing(self, enable=True):
        self.text1.set_grad_checkpointing(enable)
        self.text2.set_grad_checkpointing(enable)

    @torch.jit.ignore
    def no_weight_decay(self):
        # for timm optimizers, 1d params like logit_scale, logit_bias, ln/bn scale, biases are excluded by default
        no_wd = set()
        if hasattr(self.text1, "no_weight_decay"):
            for n in self.text1.no_weight_decay():
                no_wd.add("text1." + n)
        if hasattr(self.text2, "no_weight_decay"):
            for n in self.text2.no_weight_decay():
                no_wd.add("text2." + n)
        return no_wd

    def encode_text1(self, text, normalize: bool = False):
        features = self.text1(text)
        return F.normalize(features, dim=-1) if normalize else features

    def encode_text2(self, text, normalize: bool = False):
        features = self.text2(text)
        return F.normalize(features, dim=-1) if normalize else features

    def get_logits(self, image, text):
        image_features = self.encode_text1(image, normalize=True)
        text_features = self.encode_text2(text, normalize=True)
        image_logits = self.logit_scale.exp() * image_features @ text_features.T
        if self.logit_bias is not None:
            image_logits += self.logit_bias
        text_logits = image_logits.T
    ...
    def forward_intermediates(
        self,
        text1: Optional[torch.Tensor] = None,
        text2: Optional[torch.Tensor] = None,
        text1_indices: Optional[Union[int, List[int]]] = None,
        text2_indices: Optional[Union[int, List[int]]] = None,
        stop_early: bool = False,
        normalize: bool = True,
        normalize_intermediates: bool = False,
        intermediates_only: bool = False,
        text1_output_fmt: str = "NLC",
        text1_output_extra_tokens: bool = False,
        text2_output_fmt: str = "NLC",
        text2_output_extra_tokens: bool = False,
        output_logits: bool = False,
        output_logit_scale_bias: bool = False,
    ) -> Dict[str, Union[torch.Tensor, List[torch.Tensor]]]:
        """Forward features that returns intermediates.
        ...
        if output_logits:
            assert text1 is not None and text2 is not None, (
                "Both text1 and text2 inputs are required to compute logits"
            )

        if text1 is not None:
            text1_output = self.text1.forward_intermediates(
                text1,
                indices=text1_indices,
                stop_early=stop_early,
                normalize_intermediates=normalize_intermediates,
                intermediates_only=intermediates_only,
                output_fmt=text1_output_fmt,
                output_extra_tokens=text1_output_extra_tokens,
            )
            if normalize and "text1_features" in text1_output:
                text1_output["text1_features"] = F.normalize(
                    text1_output["text1_features"], dim=-1
                )
            output.update(text1_output)

        if text2 is not None:
            text2_output = self.text2.forward_intermediates(
                text2,
                indices=text2_indices,
                stop_early=stop_early,
                normalize_intermediates=normalize_intermediates,
                intermediates_only=intermediates_only,
                output_fmt=text2_output_fmt,
                output_extra_tokens=text2_output_extra_tokens,
            )
            if normalize and "text2_features" in text2_output:
                text2_output["text2_features"] = F.normalize(
                    text2_output["text2_features"], dim=-1
                )
            output.update(text2_output)
        ...
        if output_logits:
            text1_logits = (
                logit_scale_exp * output["text1_features"] @ output["text2_features"].T
            )
            if self.logit_bias is not None:
                text1_logits += self.logit_bias
            text2_logits = text1_logits.T
            output["text1_logits"] = text1_logits
            output["text2_logits"] = text2_logits
    ...
    def forward(
        ...
        text1_features = (
            self.encode_text1(text1, normalize=True) if text1 is not None else None
        )
        text2_features = (
            self.encode_text2(text2, normalize=True) if text2 is not None else None
        )

        if self.output_dict:
            out_dict = {
                "image_features": text1_features,
                "text_features": text2_features,
                "logit_scale": self.logit_scale.exp(),
            }
            if self.logit_bias is not None:
                out_dict["logit_bias"] = self.logit_bias
