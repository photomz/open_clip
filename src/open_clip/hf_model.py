...
class HFTextEncoder(nn.Module):
    ...
    def __init__(
        ...
        match self.config.architectures[0]:
            case "ESMplusplusForMaskedLM":
                self.config.pad_token_id = 26
            case "Qwen2ForCausalLM":
                self.config.pad_token_id = 151643
            case _:
                debug(self.config.architectures[0])
                pass
    ...
    def forward(self, x: TensorType):
        ...
        projected = self.proj(pooled_out)  # TODO: Trainable

        # print(pooled_out.shape, projected.shape)
